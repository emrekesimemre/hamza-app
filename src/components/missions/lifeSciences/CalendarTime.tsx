...
import { generateCalendarTimeSession } from '@/missions/modules/life/calendarTime/generateQuestion'
...
const CATEGORY_HINT: Record<string, string> = {
  time: 'Zaman birimlerini öğren!',
  calendar: 'Takvim bilgisini kullan!',
  season: 'Mevsimleri hatırla!',
  month: 'Ayları sırayla düşün!',
}

export function CalendarTime({ mission, difficulty, onComplete }: MissionModuleProps) {
  const generateSession = useCallback(
    (count: number, excludeKeys: string[] = []) =>
      generateCalendarTimeSession(count, difficulty, excludeKeys),
    [difficulty],
  )

  const session = useQuestionSession({
    generateQuestion: () => generateCalendarTimeSession(1, difficulty)[0],
    generateSession,
    getQuestionKey: (q) => q.prompt,
    onComplete,
    minAccuracy: mission.completionCriteria.minAccuracy,
  })

  const question = session.currentQuestion
  const hint = CATEGORY_HINT[question.category] ?? 'Doğru cevabı bul!'
  ...
  return (
    <div className="animate-bounce-in rounded-3xl bg-white p-6 shadow-xl sm:p-8">
      ...
      <div className="mb-8 rounded-2xl bg-teal-50 p-6 text-center">
        ...
        <p className="mt-4 text-xl font-extrabold leading-snug text-teal-900">{question.prompt}</p>
      </div>
